...
export class OrderService {
  ...
  public getOrders(): Observable<GetOrder[]> {
    return this.http.get<GetOrder[]>('http://localhost:8081/api/private/orders/all');
  }

  public getOrderById(id: string): Observable<GetOrder> {
    return this.http.get<GetOrder>('http://localhost:8081/api/private/orders/' + id);
  }

  public createOrder(order: CreateOrder): Observable<CreateOrder> {
    return this.http.post<CreateOrder>('http://localhost:8081/api/private/orders/create', order);
  }
